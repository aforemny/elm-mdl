...
update action model =
  case action of
    ...
    SetIcon s -> 
      ( { model | icon = if s == "" then Nothing else Just s } 
      , Cmd.none
      )
...
type alias Align = 
  ( String, Menu.Property (Button.Config Msg) Msg )


options : Model -> Align -> Parts.Index -> List (Menu.Property (Button.Config Msg) Msg)
        ...
        [ div 
            ...
            [ div 
                ...
                [ div
                    ...
                    ( if idx > 1 then [ background, bar idx (idx % 2 == 1) ]
                      else [ bar idx (idx % 2 == 1), background ]
                    )
                ]
            , div [ css "flex-grow" "1" ] [] 
            , div 
