module Material.Select.Textfield exposing 
  ( Property, label, floatingLabel, error, value, disabled, password
  , onInput, onKeyDown
  , Msg(..), Model, defaultModel, update, view, view'
  , render
...
import Html.Events exposing (onFocus, onBlur, targetValue, keyCode)
...
import Material.Select.Geometry exposing (Geometry, decodeAsTextfield)
...
type alias Config m = 
  ...
  , onKeyDown : Maybe (Html.Attribute m)
  , kind : Kind
  , rows : Maybe Int
  , cols : Maybe Int
  }
...
{-|
  TODO
-}
onKeyDown : (Int -> Int -> m) -> Property m
onKeyDown f = 
  Options.set
    (\config -> { config | onInput = 
      Just (Html.Events.on "keydown" (Decoder.object2 f numberOfItems keyCode)) })
...
-}
numberOfItems =
  Decoder.map List.length <|
  DOM.target              <|
  DOM.parentElement       <| -- .mdl-textfield
  DOM.parentElement       <| -- .mdl-select__textfield
  DOM.nextSibling         <| -- .mdl-select__menu
  DOM.childNode 0         <| -- .
  DOM.childNode 1         <| -- .mdl-menu__container
  DOM.childNode 1         <| -- .mdl-menu
  DOM.childNodes (Decoder.succeed ())
  ...
  in
    Options.apply summary div
      ...
      ([ Just <| elementFunction
          ...
          , onBlur (lift Blur)
          , Html.Events.on "focus" (Decoder.map (Focus >> lift) decodeAsTextfield)
